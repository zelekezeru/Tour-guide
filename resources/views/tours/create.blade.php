        ...
        <form action="{{ route('tours.store') }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="row">
              <div class="form-group col-lg-6">
                  ...
                  <input type="location" value="{{ old('location') ? old('location') : '' }}" name="location" class="form-control" id="location" aria-describedby="emailHelp"
                  placeholder="Enter location">
                  @if ($errors->has('location'))
                      <small id="nameHelp" class="form-text text-danger">{{$errors->first('location')}}</small>
                  @else
                      <small id="emailHelp" class="form-text text-muted">Enter Location here.</small>
                  @endif
              </div>
              <div class="form-group col-lg-4">
